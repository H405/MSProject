...
float3		positionLightPoint_[ 16 ];			// ポイントライトライトの座標
float3		colorLightPoint_[ 16 ];				// ポイントライトライトの色
float3		attenuationLightPoint_[ 16 ];		// ポイントライトの減衰率
...
PixelOutput DrawPixel( VertexOutput vertex )
{
	...
	// ディレクショナルライトのディフューズ色を計算
	diffusePoint = colorLightPoint_[ 9 ] * min( dot( vectorNormal, -vectorLightToVertex ) * 0.5f + 0.5f, shadow );
	...
	// ディレクショナルライトのディフューズ色を計算
	diffusePoint = colorLightPoint_[ 10 ] * min( dot( vectorNormal, -vectorLightToVertex ) * 0.5f + 0.5f, shadow );
	...
	// ディレクショナルライトのディフューズ色を計算
	diffusePoint = colorLightPoint_[ 11 ] * min( dot( vectorNormal, -vectorLightToVertex ) * 0.5f + 0.5f, shadow );
	...
	// ディレクショナルライトのディフューズ色を計算
	diffusePoint = colorLightPoint_[ 12 ] * min( dot( vectorNormal, -vectorLightToVertex ) * 0.5f + 0.5f, shadow );
	...
	// ディレクショナルライトのディフューズ色を計算
	diffusePoint = colorLightPoint_[ 13 ] * min( dot( vectorNormal, -vectorLightToVertex ) * 0.5f + 0.5f, shadow );
	...
	// ディレクショナルライトのディフューズ色を計算
	diffusePoint = colorLightPoint_[ 14 ] * min( dot( vectorNormal, -vectorLightToVertex ) * 0.5f + 0.5f, shadow );
	...
	// ポイントライトの減衰率を計算
	distanceLightToVertex = distance( positionWorld, positionLightPoint_[ 15 ] );
	attenuation = attenuationLightPoint_[ 15 ].x
		+ attenuationLightPoint_[ 15 ].y * distanceLightToVertex
		+ attenuationLightPoint_[ 15 ].z * distanceLightToVertex * distanceLightToVertex;

	// ライトから頂点へのベクトルを求める
	vectorLightToVertex = normalize( positionWorld - positionLightPoint_[ 15 ] );

	// ディレクショナルライトのディフューズ色を計算
	diffusePoint = colorLightPoint_[ 15 ] * min( dot( vectorNormal, -vectorLightToVertex ) * 0.5f + 0.5f, shadow );

	// ポイントライトのスペキュラ色を計算
	specularPoint = CalculateSpecular( colorLightPoint_[ 15 ], vectorLightToVertex, vectorNormal, vectorVertexToEye, dataDiffuse.a, power );

	// ポイントライトのリム色を計算
	rimPoint = CalculateRim( colorLightPoint_[ 15 ], vectorLightToVertex, vectorNormal, vectorVertexToEye );
