...
float3		positionLightPoint_[ 10 ];			// ポイントライトライトの座標
float3		colorLightPoint_[ 10 ];				// ポイントライトライトの色
float3		attenuationLightPoint_[ 10 ];		// ポイントライトの減衰率
...
//	normal = vertex.vectorNormalWorld_;
	...
	float3	colorDiffuse = colorDiffuse_;//tex2DProj( samplerTextureEnvironmentFront, vertex.positionTexture_ );
	...
	// ポイントライトの減衰率を計算
	distanceLightToVertex = distance( vertex.positionWorld_, positionLightPoint_[ 9 ] );
	...
	// ライトから頂点へのベクトルを求める
	vectorLightToVertex = normalize( vertex.positionWorld_ - positionLightPoint_[ 9 ] );

	// ディレクショナルライトのディフューズ色を計算
	diffusePoint = CalculateDiffuse( colorLightPoint_[ 9 ], vectorLightToVertex, normal );

	// ポイントライトのスペキュラ色を計算
	specularPoint = CalculateSpecular( colorLightPoint_[ 9 ], vectorLightToVertex, normal, vectorVertexToEye, reflection_, power_ );

	// ポイントライトのリム色を計算
	rimPoint = CalculateRim( colorLightPoint_[ 9 ], vectorLightToVertex, normal, vectorVertexToEye );
